let profilePage = {
    ...
    messages: {
        ...
        errorCodiceNotInserted: "<p>Inserire il codice del Profilo</p>",
        errorDescrNotInserted: "<p>Inserire la descrizione del Profilo</p>",
     ...
     */
    initDataInsertProfilePage: async function () {
		showLoading();
        this.initCommon();
        ...
        $('#addRoleBtn').click(() => this.addRoleProfilePrototype());
     ...
     * @returns {Promise<void>} promise di fine caricamento pagina.
     */
    initDataEditProfilePage: async function () {
		showLoading();
        this.initCommon();
     ...
     * l'operatore non ha i permessi di rimuovere/modificare quel ruolo).
     */
    addRoleProfilePrototype: function (readonly) {
        let inputs = $('#rolesContainer select[name="ruoli"]');
        if ($(inputs).filter((i, e) => $(e).val() === '').length > 0) {
            this.showAlert(this.messages.errorFieldsNotFilled, null, 'error');
            return;
        ...
        let inserted = this.insertPrototype('rolesContainer', 'role');
        $(inserted).find('select.role-selector')
            .focus(e => $(e.target).data('previous', $(e.target).val()))
            .change(e => this.selectRole(e.target, false));
         if (readonly) {
   ...
   /* deleteRole: function (button) {
	...
	/*
		let codice = $('input[name="codice"]').val()
		if(codice == null || codice == '' || codice.trim().length === 0) {
			this.showAlert(this.messages.errorCodiceNotInserted, 'Campo obbligatorio', 'error');
			return;
		}
		let descr = $('input[name="descrizione"]').val()
		if(descr == null || descr == '' || descr.trim().length === 0) {
			this.showAlert(this.messages.errorDescrNotInserted, 'Campo obbligatorio', 'error');
			return;
		}
        let collocations = $('#collocationContainer')
            .find('input[name="collocazioni"]')
     ...
     * @param type il tipo del modale. Puo' essere: info - warning - error
     */
      showAlertScerev: async function (msg) {
        let cssClass;
           cssClass = 'alert-info';
           title = msg=='abilitazioneScerev' ? 'Abilitazione Scerev' : 'Disabilitazione Scerev';

           let messaggio = await $.ajax({
            url: '/configuratore/ajax/collocazione/messaggio',
            dataType: 'json',
            data: {msg: msg},
            method: 'POST'
        });
