...
/**
 */
@WebService(serviceName = "AuthenticationService", portName = "AuthenticationService", targetNamespace = "http://dmacc.csi.it/", endpointInterface = "it.csi.dma.dmaloginccebl.interfacews.authentication.AuthenticationService")
@Service(value = "authenticationService")
public class AuthenticationServiceImpl implements AuthenticationService {

    private final static Logger log = Logger .getLogger(Constants.APPLICATION_CODE);

	private static final Object GETAUTHENTICATION = "GetAuthentication";
    ...
    @Autowired
    AuthenticationServiceValidator authenticationServiceValidator;
    ...
    @Autowired
    VerifyLoginConditionsClient verifyLoginConditionsClient;
	...
	public GetAuthenticationResponse getAuthentication(GetAuthenticationRequest parameters) {
        final Richiedente richiedente = parameters.getRichiedente();

        List<Errore> errori = new ArrayList<>();
        GetAuthenticationResponse response = null;
        ...
        try {
            ...
            //TODO Verifica certificato STEP 1 (DA RICEVERE INFORMAZIONI AGGIUNTIVE)

        	/* LOG START */

            String certificato = getCertificatoFromSecurity();


            logGeneralDaoBean = this.prepareLogBeanAuthentication(parameters, certificato);
            ...
            if (checkErrori(errori)) return response = new GetAuthenticationResponse(errori, RisultatoCodice.FALLIMENTO);
            ...
            /* Verfica credenziali IRIDE */

                //Chiamata ai servizi di IRIDE, se PIN non richiesto solo per username e password
                //Altrimenti si chiama per username password e PIN

                //Bisogna salvare XML_IN e XML_OUT della chiamata nella tabella lcce.auth_l_xml_servizi_richiamati
                //Errore in caso di risposta negativa AUTH_ER_501
            Credenziali credenziali = richiedente.getCredenziali();

            Identita identitaIride = authService.autenticaMedico(credenziali, applicazioneDto, logGeneralDaoBean);
            if(identitaIride.getErrore() != null){
                errori.add(identitaIride.getErrore());
                return response = new GetAuthenticationResponse(errori,RisultatoCodice.FALLIMENTO);
            }
            //sb jira LCCE-7
            logGeneralDaoBean.getMessaggiDto().setCfRichiedente(identitaIride.getCodFiscale());
            ...
            UtenteDto utenteDto = authenticationServiceValidator.verificaIrideResponse(logGeneralDaoBean, errori, identitaIride);
            if (checkErrori(errori)) return response = new GetAuthenticationResponse(errori, RisultatoCodice.FALLIMENTO);
            ...
            if (checkErrori(errori)) return response = new GetAuthenticationResponse(errori,
            ...
            if(applicazioneDto.getUrlVerifyLoginConditions() != null){
                ...
                if(verifyLoginConditionsResponse.getEsito() == RisultatoCodice.FALLIMENTO){
                    ...
                    return response = new GetAuthenticationResponse(erroriOrigResponse,
                            RisultatoCodice.FALLIMENTO);
                }
            ...
            LoginDataDto loginDataDto = registraLoginData(parameters, identitaIride, abilitazioneDto, tokenAutenticazione);
            ...
            response = new GetAuthenticationResponse(null, RisultatoCodice.SUCCESSO);
            ...
            errori.add(logGeneralDao.logErrore(logGeneralDaoBean.getLogDto(),
                    CatalogoLog.ERRORE_INTERNO.getValue()));
            response = new GetAuthenticationResponse(errori,
                    RisultatoCodice.FALLIMENTO);
        } finally {
            /* LOG END */
    ...
    private String getParametriLogStart(GetAuthenticationRequest parameters) {
    ...
    private VerifyLoginConditionsRequest getVerifyLoginConditionsRequest(GetAuthenticationRequest parameters, Richiedente richiedente, UtenteDto utenteDto) {
    ...
    private void setListaParametriLogin(GetAuthenticationRequest parameters, VerifyLoginConditionsRequest verifyLoginConditionsRequest, List<it.csi.dma.dmaloginccebl.client.verifyLoginConditions.common.ParametriLogin> listParametriVerify) {
        for(ParametriLogin parametro : parameters.getParametriLogin()){
    ...
    private LoginDataDto registraLoginData(GetAuthenticationRequest parameters, Identita identitaIride, AbilitazioneDto abilitazioneDto, String tokenAutenticazione) {
        LoginDataDto loginDataDto = getLoginDataDto(parameters, identitaIride, abilitazioneDto, tokenAutenticazione);

        return loginDataLowDao.insert(loginDataDto);
    }

    private LoginDataDto getLoginDataDto(GetAuthenticationRequest parameters, Identita identitaIride, AbilitazioneDto abilitazioneDto, String tokenAutenticazione) {
        LoginDataDto loginDataDto = new LoginDataDto();
        loginDataDto.setAbilitazioneDto(abilitazioneDto);
        loginDataDto.setCfRichiedente(identitaIride.getCodFiscale());
	...
	private LogGeneralDaoBean prepareLogBeanAuthentication(GetAuthenticationRequest getAuthenticationRequest, String certificato) {
		ServiziDto authenticationService = Utils.getFirstRecord(serviziLowDao.findByCodice(new ServiziDto(), Servizi.AUTHENTICATION.getValue()));
	...
	private LogAuditDto createLogAuditDto(GetAuthenticationRequest getAuthenticationRequest,
	...
	private LogDto createLogDto(GetAuthenticationRequest getAuthenticationRequest, ServiziDto authenticationService) {
	...
	private MessaggiDto createMessaggiDto(GetAuthenticationRequest getAuthenticationRequest,
			ServiziDto authenticationService, Richiedente richiedente, String certificato) {
		...
		messaggiDto.setCertificato(certificato);
		return messaggiDto;
	}
